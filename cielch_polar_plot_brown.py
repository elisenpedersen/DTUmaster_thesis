"""
Clean Brown Modules Analysis - New Version
This script generates only the essential plots for brown module analysis:
1. Connected data plots for each brown module
...
BROWN_MODULES = [18, 19, 20, 21, 22, 23]
MODULE_DESCRIPTIONS = {
    18: "Module 18 (BrownC)", 
    19: "Module 19 (BrownCx)", 
    20: "Module 20 (BrownBaC)", 
    21: "Module 21 (BrownBaCx)", 
    22: "Module 22 (BrownGlC)", 
    23: "Module 23 (BrownGlCx)"
}
...
def plot_brown_module_connected_data(df, module_id, output_dir):
    """
    Create connected data plot for a single brown module.
    ...
    plt.savefig(os.path.join(output_dir, f'brown_module_{module_id:02d}_connected_data.png'), dpi=300, bbox_inches='tight')
    plt.close()

def plot_brown_modules_overlayed_raw(df, output_dir):
    """
    Create overlayed CIELCH polar plot showing all brown modules with different grey colors for incident angles.
    """
    fig, ax = plt.subplots(subplot_kw={'projection': 'polar'}, figsize=(12, 10))
    
    # Create standardized LCH plot
    create_lch_polar_plot(ax, "Brown Modules: Data Points by Incident Angle\n(Grey colors = angles, Circles = all modules)")
    ...
    for angle_idx, angle in enumerate(all_angles):
        ...
        for module_idx, module in enumerate(BROWN_MODULES):
    ...
    plt.tight_layout(rect=[0, 0, 0.85, 0.97])
    plt.savefig(os.path.join(output_dir, 'brown_modules_overlayed_raw.png'), dpi=300, bbox_inches='tight')
    plt.close()

def plot_hue_trajectory_comparison(df, output_dir):
    """
    Compare hue trajectories across brown modules to see if they follow similar patterns.
    """
    ...
    for idx, module in enumerate(BROWN_MODULES):
        module_data = df[df['Module'] == module]
        if not module_data.empty:
...
def main():
    ...
    print("BROWN MODULES ANALYSIS - NEW VERSION")
    print("=" * 50)
    
    # Create output directory
    output_dir = os.path.join(OUTPUT_DIR, 'brown_modules_analysis_new')
    os.makedirs(output_dir, exist_ok=True)
    
    print("1. Generating connected data plots for each brown module...")
    for module in BROWN_MODULES:
        print(f"   Processing {MODULE_DESCRIPTIONS[module]}...")
        plot_brown_module_connected_data(df, module, output_dir)
    
    print("2. Generating overlayed raw data plot...")
    plot_brown_modules_overlayed_raw(df, output_dir)
    ...
    print("\nGenerated files:")
    for module in BROWN_MODULES:
        print(f"  - brown_module_{module:02d}_connected_data.png")
    print("  - brown_modules_overlayed_raw.png")
