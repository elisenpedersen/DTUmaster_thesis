...
#script to plot all the brdf plots for all the modules logarithmic and with a visible light colormap
# saves plots to a folder called brdf_plots for each module folder
# Emphasizes where reflection is concentrated and its spread.
...
def plot_brdf_2d(results_df, name=None):
    """
    ...
    name : str, optional
        Name to use in plot title
    """
    if results_df is None or len(results_df) == 0:
        print(f"No data to plot")
        return
        
    if 'total_brdf' not in results_df.columns:
        print(f"No BRDF data found in results")
    ...
    print(f"BRDF range: {results_df['total_brdf'].min():.3f} to {results_df['total_brdf'].max():.3f}")
    
    # Create figure
    fig = plt.figure(figsize=(12, 8))
    theta_i = results_df['theta_i'].iloc[0]
    phi_i = results_df['phi_i'].iloc[0]
    
    # Create polar plot
    ax = plt.subplot(111, projection='polar')
    
    # Convert angles to radians for plotting
    theta = np.radians(results_df['phi_r'])
    r = results_df['theta_r']
    brdf = results_df['total_brdf']
    
    # Create scatter plot with logarithmic BRDF values
    scatter = ax.scatter(theta, r, c=np.log10(brdf), cmap='viridis')
    
    # Add colorbar with proper labels
    cbar = plt.colorbar(scatter, label='log₁₀(BRDF)')
    
    # Set title and labels
    title = f'BRDF at θᵢ={theta_i}°, φᵢ={phi_i}°'
    if name:
        title = f'{name} - {title}'
    ax.set_title(title)
...
def plot_brdf_3d(results_df, name=None):
    """
    ...
    # Use logarithmic BRDF values for plotting
    log_brdf = np.log10(brdf)
    x = log_brdf * np.sin(theta_r) * np.cos(phi_r)
    y = log_brdf * np.sin(theta_r) * np.sin(phi_r)
    z = log_brdf * np.cos(theta_r)
    
    # Create scatter plot
    scatter = ax.scatter(x, y, z, c=log_brdf, cmap='viridis')
    ...
    theta_i = results_df['theta_i'].iloc[0]
    phi_i = results_df['phi_i'].iloc[0]
    title = f'3D BRDF at θᵢ={theta_i}°, φᵢ={phi_i}°'
    if name:
        title = f'{name} - {title}'
    ax.set_title(title)
    ax.set_xlabel('X')
...
# Define incident angles to process
incident_angles = [0, 15, 30, 45, 60]

# Process data for all modules
for module_num in range(1, 30):  # Process modules 1-29
    module_name = f'Module{module_num}'
    module_path = os.path.join(meas_folder, module_name)
    
    # Skip if module directory doesn't exist
    if not os.path.exists(module_path):
        print(f"\nSkipping {module_name} - directory not found")
        continue
    
    print(f"\nProcessing {module_name}")
    
    # Create module-specific output directory
    module_output_dir = os.path.join('brdf_plots', module_name)
    os.makedirs(module_output_dir, exist_ok=True)
    
    # Process each incident angle
    for theta_i in incident_angles:
        print(f"\nProcessing θᵢ={theta_i}°")
        
        # Get all measurement files for the specified theta_i
        measurement_files = [f for f in os.listdir(module_path) 
                           if f.startswith(f'ti_{theta_i}_') and f.endswith('.csv')]
        
        if not measurement_files:
            print(f"No measurements found for θᵢ={theta_i}°")
            continue
        ...
        # Calculate BRDF
        results_df = calculate_brdf(module_path, df, dark, dark_time,
                                  caldata, cal_time, int_range,
                                  white_irradiance_reference_mean,
                                  white_spec_irradiance_reference_mean)
        
        if results_df is not None and len(results_df) > 0:
            print(f"Successfully calculated BRDF for {len(results_df)} measurements")
            print(f"BRDF range: {results_df['total_brdf'].min():.3f} to {results_df['total_brdf'].max():.3f}")
            
            # Save pickle file
            pkl_path = os.path.join(module_output_dir, f'{module_name}_theta{theta_i}_phi0.pkl')
            with open(pkl_path, 'wb') as f:
                pickle.dump(results_df, f)
            print(f"Saved BRDF data to {pkl_path}")
            
            # Create plots
            print(f"\nGenerating plots for {module_name} at θᵢ={theta_i}°")
            
            # 2D plot
            fig, ax = plot_brdf_2d(results_df, module_name)
            plt.savefig(os.path.join(module_output_dir, f'{module_name}_theta{theta_i}_phi0_2d.png'), 
                       dpi=300, bbox_inches='tight')
            plt.close(fig)
            
            # 3D plot
            fig, ax = plot_brdf_3d(results_df, module_name)
            plt.savefig(os.path.join(module_output_dir, f'{module_name}_theta{theta_i}_phi0_3d.png'), 
                       dpi=300, bbox_inches='tight')
            plt.close(fig)
            
        else:
            print(f"No valid BRDF results for {module_name} at θᵢ={theta_i}°")

# Turn off interactive mode and close all plots
plt.ioff()
plt.close('all')
